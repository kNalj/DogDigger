...
class BazosSkSpider(scrapy.Spider):
    name = 'bazos_sk'
    allowed_domains = ['zvierata.bazos.sk']
    start_urls = ['https://zvierata.bazos.sk/pes/']
    ...
    def parse(self, response):
        ...
        for add in response.css(add_selector):
            ...
            dog_add["link"] = "https://zvierata.bazos.sk" + add.css(link_selector).extract_first()
        ...
        for page in [response.css(next_page_selector).getall()[-1]]:
            ...
            if int(page_size) <= 200:
                ...
                if link_text == "Ďalšia":
                    ...
                    yield response.follow("https://zvierata.bazos.sk" + page, self.parse)
